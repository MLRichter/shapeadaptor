...
if __name__ == '__main__':
    ...
    if args.network == 'better_vgg':
        model = VGG(type='D3', sa_num=args.sa_num,
                    dataset=args.dataset, mode=args.mode,
                    input_shape=args.input_dim, output_shape=args.output_dim).to(device)
        ...
        model = ResNet(Bottleneck, [3, 4, 6, 3], sa_num=args.sa_num,
                       dataset=args.dataset, mode=args.mode,
                       input_shape=args.input_dim, output_shape=args.output_dim).to(device)
    elif args.network == 'better_resnet':
        model = ResNet(Bottleneck, [3, 4, 6, 3], strides=[1, 1, 2, 1], sa_num=args.sa_num,
                       dataset=args.dataset, mode=args.mode,
                       input_shape=args.input_dim, output_shape=args.output_dim).to(device)
    ...
    wandb.init(project="Adaptive Shape",
               config=args)
    ...
    for index in range(total_epoch):
        ...
        for i in tqdm(list(range(train_batch))):
        ...
        wandb.log(
            {
                "epoch": index,
                "step": iteration,
                "train loss": avg_cost[index][0],
                "train acc": avg_cost[index][1],
                "test loss": avg_cost[index][2],
                "test acc": avg_cost[index][3],
                "MACs": flops,
                "params": params
            }
        )
    ...
    if 'human' in args.mode:
        ...
        dict = {'shape-list': shape_list,
                ...
                'parms': params,
                'time': end_time - start_time}
        np.save('{}_{}_{}_sanum{}_outputdim{}_{}.npy'
