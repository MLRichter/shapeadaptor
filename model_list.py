...
class VGG(nn.Module):
    def __init__(self, input_shape=32, output_shape=8, dataset=None, mode=None, sa_num=None, type='D', better=False):
        ...
        self.filter = {
            ...
            'D3': [64, 64, 128, 128, 256, 256, 256, 512, 512, "M", 512, 512, 512, 512],
...
class ResNet(nn.Module):
    def __init__(self, block, layers, dataset, mode, input_shape=32, output_shape=8, sa_num=None, strides=[1, 2, 2, 2]):
        ...
        self.layer1 = self._make_layer(block, 64,  self.layers[0], stride=strides[0])
        self.layer2 = self._make_layer(block, 128, self.layers[1], stride=strides[1])
        self.layer3 = self._make_layer(block, 256, self.layers[2], stride=strides[2])
        self.layer4 = self._make_layer(block, 512, self.layers[3], stride=strides[3], endlayer=True)
...
if __name__ == '__main__':
    ...
    model = VGG(better=True, mode="cifar-human", dataset="cifar100", type="D3")#ResNet(Bottleneck, [3, 4, 6, 3], strides=[1, 1, 2, 1], sa_num=None,
